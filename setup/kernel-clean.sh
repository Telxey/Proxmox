...
echo -e "${CL}${YW}Hello${GN} Today is: ${WT} `date` `time` ${CL}"
echo -e "${DM}Author: ${WT}(${RD} Rax ${CL${WT}) ${GN}${BLD}TELXEY${CL}"
echo -e "${CL}"
...
select yn in "Yes" "No"; do
    case $yn in
        Yes ) $@; break;;
        No ) echo "${RD}Kernel Cleaner Canceled${CL}";  exit;;
    esac
done
...
whiptail --backtitle "Proxmox Scripts" --title "Proxmox Kernel Clean" --yesno "This will Clean Unused Kernel Images, USE AT YOUR OWN RISK. Proceed?" 10 68 || exit
